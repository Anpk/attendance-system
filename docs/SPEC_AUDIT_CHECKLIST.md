...
| Item | Check | Status | Notes |
|-----|------|--------|------|
| 90-errors.md 포맷 준수 | timestamp/status/error/code/message/path | [PASS] | ApiErrorResponse 6필드로 확장 완료 / Smoke(2026-01-22): today(empty snapshot) → check-in(checkInAt set) → check-out(checkOutAt set), duplicate check-out → 409 ALREADY_CHECKED_OUT, missing X-USER-ID → 401 UNAUTHORIZED (all 6 fields) |
